...
(() => {
  ...
  modal.innerHTML = `<div class="counting-sheet"><header class="counting-sheet-head"><div><h2>Comptage</h2><p id="countingContext"></p></div><button class="counting-sheet-close" type="button" aria-label="Fermer">×</button></header><nav class="counting-tabs" aria-label="Mode de comptage"><button type="button" class="active" data-counting-tab="ai">Comptage IA</button><button type="button" data-counting-tab="manual">Saisie manuelle</button></nav><div id="countingAI" class="counting-tab-panel"></div><div id="countingManual" class="counting-tab-panel hidden"></div></div>`;
  document.body.append(modal);
  const aiWorkspace = modal.querySelector('#countingAI');
  const manualWorkspace = modal.querySelector('#countingManual');
  [photoCard, batch].filter(Boolean).forEach(element => aiWorkspace.append(element));
  [linesCard, checklistCard, save].filter(Boolean).forEach(element => manualWorkspace.append(element));
  const selectCountingTab = tab => { modal.querySelector('#countingAI').classList.toggle('hidden', tab !== 'ai'); modal.querySelector('#countingManual').classList.toggle('hidden', tab !== 'manual'); modal.querySelectorAll('[data-counting-tab]').forEach(button => button.classList.toggle('active', button.dataset.countingTab === tab)); };
  modal.querySelector('.counting-tabs').addEventListener('click', event => { const button = event.target.closest('[data-counting-tab]'); if (button) selectCountingTab(button.dataset.countingTab); });
  const context = modal.querySelector('#countingContext');
  const open = () => { context.textContent = `${document.querySelector('#bar')?.value || ''} · ${document.querySelector('#fridge')?.value || ''}`; selectCountingTab('ai'); modal.classList.remove('hidden'); document.body.style.overflow = 'hidden'; modal.querySelector('#camera, #nextFridgePhoto')?.focus(); };
